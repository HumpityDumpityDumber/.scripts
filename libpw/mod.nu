...
def getBookmarkUrls [] {
    mut bookmarks = []
    $bookmarks ++= (
        open ($env.PW_CACHEDIR)/bookmarks-list.json | get pixiv | each { |page|
...
export def "token refresh" [code: string, verifier: string] {
    ...
    {
        user_id: ($response.user.id | into int)
...
export def "update-bookmarks" [user_id: int, access_token: string] {
    ...
    def requestPixivBookmarks [url: string] { http get $url --headers { Authorization: $"Bearer ($access_token)" } }
    mut response = {
        next_url: $"https://app-api.pixiv.net/v1/user/bookmarks/illust?user_id=($user_id)&restrict=public&filter=for_ios&tag=wallpaper"
    }
    mut pixivToSave = []
    loop {
        $response = requestPixivBookmarks $response.next_url
        $pixivToSave ++= $response.illusts
        if $response.next_url == null { break }
    }
    mut wallhavenToSave = []
    for id in (http get "https://wallhaven.cc/api/v1/collections/raurutuchr/2091116" | get data.id) { $wallhavenToSave ++= [
        (http get https://wallhaven.cc/api/v1/w/($id) | get data.path)
    ] }
    {
        "pixiv": $pixivToSave
        "wallhaven": $wallhavenToSave
    } | save -f $bookmarks_file
    let pixivBookmarks = (getBookmarkUrls | each { url parse |
        get path |
        path basename })
    let images = (ls -s ($env.PW_CACHEDIR)/images | get name)
...
export def "get-wallpaper" [wallpaper_url: string, access_token: string] {
    ...
    if not ($file | path exists) {
        if ($wallpaper_url | url parse).host == "i.pximg.net" {
            ...
            # fetch normally if not hosted on pixiv
